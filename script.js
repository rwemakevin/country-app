...
function updateDOM() {
  ...
  fetch(apiUrlByContinent)
    ...
    .then(data => {
      ...
      function handleClick2(argument) {
        console.log("Clicked with argument:", argument);
        // Your code logic using the argument goes here
      
      let apiUrlByCountry2 = `https://restcountries.com/v3.1/name/${argument}?fullText=true`
      fetch(apiUrlByCountry2)
      .then(response => {
        if (!response.ok) {
          throw new Error(`Network response was not ok: ${response.status}`);
        }
      ...
      for (const country of data) {
        ...
        countryDiv.addEventListener('click',function(){
          handleClick2(country.name.common)
        })
